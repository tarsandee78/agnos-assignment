 ...
 * - Independent language toggle persisted in localStorage.
 */
export default function StaffPage() {
  // Subscribe to real-time room events (Supabase + BroadcastChannel fallback)
  useStaffRealtime();

  const { lang, setLang, t } = useLanguage('agnos_lang_staff', 'th');
  ...
  return (
    <div className="min-h-screen bg-background p-4 sm:p-6 lg:p-8 font-sans text-foreground">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        {isSubmitted && (
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 rounded-xl border border-emerald-300 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/40 p-4 text-emerald-950 dark:text-emerald-50 shadow-xs animate-in fade-in-50">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-sm sm:text-base text-emerald-950 dark:text-emerald-50">
                  {t.staff.submittedSuccess}
                </p>
                <p className="text-xs text-emerald-800/90 dark:text-emerald-200/90">
                  {t.staff.submittedAt}: {submittedAt ? new Date(submittedAt).toLocaleTimeString() : 'Just now'}
                </p>
              </div>
            </div>
        ...
        <PatientOverviewCards lang={lang} t={t} />
      </div>
    </div>
  );
}
